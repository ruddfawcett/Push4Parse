...
- (void)viewDidLoad
{
    ...
    self.title = [self.applicationAtIndex objectForKey:@"AppName"];
}
...
- (BOOL)textField:(UITextField *)aTextField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (aTextField == self.applicationTitle) {
        self.title = [self.applicationTitle.text stringByReplacingCharactersInRange:range withString:string];
    }
    return YES;
}
...
- (void)saveApplicationInformation {
    if (self.applicationTitle.text.length == 0 || self.apiKey.text.length == 0 || self.appID.text.length == 0) {
        ...
        [plistData replaceObjectAtIndex:[plistData indexOfObject:self.applicationAtIndex] withObject:information];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        self.applicationTitle.text = [self.applicationAtIndex objectForKey:@"AppName"];
        ...
        self.apiKey.text = [self.applicationAtIndex objectForKey:@"AppAPI"];
        ...
        self.appID.text = [self.applicationAtIndex objectForKey:@"AppID"];
